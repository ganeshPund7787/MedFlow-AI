...
export const analyzeXRayJob = inngest.createFunction(
  { id: "analyze-xray", triggers: [{ event: "labResult/created" }] },
  async ({ event, step }) => {
    const { labResultId, imageUrl, bodyPart } = event.data;

    const updatedLab = await step.run("analyze-and-save", async () => {
      const updatedLabResult = await completeXrayLabResult(
        labResultId,
        imageUrl,
        bodyPart,
      );
      ...
      const patient = await mongoose.connection.collection("user").findOne(
        { _id: new mongoose.Types.ObjectId(String(updatedLabResult.patient)) },
    ...
    await step.run("send-notification", async () => {
      const doctorId = updatedLab?.patient?.assignedDoctorId;
      const nurseId = updatedLab?.patient?.assignedNurseId;
      await notifyUsers(
